...
var _0n, _1n, hasHexBuiltin2, hexes2, asciis2, isPosBig = (n) => typeof n === "bigint" && _0n <= n, bitMask = (n) => (_1n << BigInt(n)) - _1n, u8n = (len) => new Uint8Array(len), u8fr = (arr) => Uint8Array.from(arr), validatorFns;
...
function decodeNumber(cursor, param) {
  ...
  const value = cursor.readBytes(32);
  return [
    size2 > 48 ? bytesToBigInt(value, { signed }) : bytesToNumber2(value, { signed }),
    32
  ];
}
...
var init_contract = __esm(() => {
  ...
  CallExecutionError = class CallExecutionError extends BaseError2 {
    constructor(cause, { account: account_, docsPath: docsPath3, chain: chain2, data, gas, gasPrice, maxFeePerGas, maxPriorityFeePerGas, nonce, to, value, stateOverride }) {
...
var require_cryptoNode = __commonJS((exports) => {
  ...
  exports.crypto = nc2 && typeof nc2 === "object" && "webcrypto" in nc2 ? nc2.webcrypto : nc2 && typeof nc2 === "object" && ("randomBytes" in nc2) ? nc2 : undefined;
});
...
var require_utils = __commonJS((exports) => {
  ...
  var hasHexBuiltin4 = /* @__PURE__ */ (() => typeof Uint8Array.from([]).toHex === "function" && typeof Uint8Array.fromHex === "function")();
...
var require_lib = __commonJS((exports) => {
  ...
  function convertRadix22(data, from14, to, padding3) {
    ...
    if (radix2carry2(from14, to) > 32) {
      throw new Error(`convertRadix2: carry overflow from=${from14} to=${to} carryBits=${radix2carry2(from14, to)}`);
    }
    let carry = 0;
    let pos = 0;
  ...
  var createBase58check2 = (sha2564) => chain2(checksum4(4, (data) => sha2564(sha2564(data))), exports.base58);
  ...
  exports.hex = hasHexBuiltin4 ? hexBuiltin2 : chain2(radix22(4), alphabet2("0123456789abcdef"), join2(""), normalize3((s) => {
...
var require_bip39 = __commonJS((exports) => {
  ...
  function mnemonicToSeed(mnemonic, passphrase = "") {
    return (0, pbkdf2_1.pbkdf2Async)(sha2_1.sha512, normalize3(mnemonic).nfkd, psalt2(passphrase), { c: 2048, dkLen: 64 });
  }
  function mnemonicToSeedSync2(mnemonic, passphrase = "") {
    return (0, pbkdf2_1.pbkdf2)(sha2_1.sha512, normalize3(mnemonic).nfkd, psalt2(passphrase), { c: 2048, dkLen: 64 });
  }
});
...
function createClient(parameters) {
  ...
  const experimental_blockTag = parameters.experimental_blockTag ?? (typeof chain2?.experimental_preconfirmationTime === "number" ? "pending" : undefined);
...
async function verifyHash(client, parameters) {
  const { address, chain: chain2 = client.chain, hash: hash3, erc6492VerifierAddress: verifierAddress = parameters.universalSignatureVerifierAddress ?? chain2?.contracts?.erc6492Verifier?.address, multicallAddress = parameters.multicallAddress ?? chain2?.contracts?.multicall3?.address } = parameters;
...
var THINKING_MODEL_DEFAULT_TOKENS = 2000;
var STANDARD_MODEL_DEFAULT_TOKENS = 256;
...
`);
  ...
  async complete(message, options) {
    ...
    if (maxTokens === undefined) {
      maxTokens = THINKING_MODELS.has(model) ? THINKING_MODEL_DEFAULT_TOKENS : STANDARD_MODEL_DEFAULT_TOKENS;
    }
...
class MorDiemSDK {
  ...
  constructor(config = {}) {
    ...
    if (config.mnemonic) {
      ...
      this._privateKey = config.privateKey.startsWith("0x") ? config.privateKey : `0x${config.privateKey}`;
    }
